...
hlm <- function(nsamples, burnin, y, group.vector, a_alpha, b_alpha, a_beta, b_beta, lambda.curr, alpha.curr, beta.curr){
  ...
  "sample.lambda" <- function(nsamples, J, alpha, beta, sigma.y, n){
    ...
    return(sapply(1:J, function(x) rgamma(n = nsamples, lambda_alpha[x], lambda_beta[x])))
  }
